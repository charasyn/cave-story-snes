...
void player_draw() {
	...
	if(playerMoveMode) {
		...
		if(player.grounded) {
			if(joy_down(KEY_UP) && !controlsLocked) {
				lookingDown = FALSE;
				if(joystate&(KEY_LEFT|KEY_RIGHT)) {
					const uint8_t f[] = { UPWALK1, LOOKUP, UPWALK2, LOOKUP };
			...
			} else if(joystate&(KEY_LEFT|KEY_RIGHT) && !controlsLocked) {
				lookingDown = FALSE;
				const uint8_t f[] = { WALK1, STAND, WALK2, STAND };
				if(++player.timer2 > TIME_8(5)) {
					player.timer2 = 0;
					if(++player.animtime > 3) player.animtime = 0;
				}
				player.frame = f[player.animtime];
			} else if(joy_pressed(KEY_DOWN) && !controlsLocked) {
				lookingDown = TRUE;
				player.frame = LOOKDN;
				player.animtime = 0;
			} else if(!lookingDown) {
			...
			} else if(joy_down(KEY_DOWN) && !controlsLocked) {
				player.frame = JUMPDN;
			} else {
...
void player_update() {
	...
	if(!playerMoveMode) { // Normal movement
		...
		if(playerBoostState == BOOST_OFF && bx > 0 && bx < stageWidth - 1 && by > 0 && by < stageHeight - 1) {
			...
			// into small gaps
			if(!blockl_next && blockl && joy_down(KEY_LEFT)) {
				player.x_speed -= 0xE0;
				player.x_next = player.x + player.x_speed;
			}
			if(!blockr_next && blockr && joy_down(KEY_RIGHT)) {
				player.x_speed += 0xE0;
				player.x_next = player.x + player.x_speed;
			}
			if(ledge_time > 0) {
	...
	/*if(cfg_updoor ? joy_pressed(KEY_UP) : joy_pressed(KEY_DOWN)) {
		...
		while(e) {
			if((e->flags & NPC_INTERACTIVE) && entity_overlapping(&player, e)) {
				...
				oldstate |= cfg_updoor ? KEY_UP : KEY_DOWN;
				if(e->event > 0) {
...
static void player_update_booster() {
	...
	if (joy_down(KEY_LEFT)) player.dir = 0;
	else if (joy_down(KEY_RIGHT)) player.dir = 1;
	...
	switch(playerBoostState) {
		case BOOST_HOZ:
		{
			...
			//	if(player.y_speed < 0) player.y_speed += SPEED(0x20);
			//}
			//if (joy_down(KEY_DOWN)) player.y_speed += SPEED(0x20);
			//if (joy_down(KEY_UP)) player.y_speed -= SPEED(0x20);
		}
		break;
		case BOOST_UP:
		{
...
static void player_prev_weapon() {
	int16_t i;
	for( i = currentWeapon - 1; i % MAX_WEAPONS != currentWeapon; i--) {
